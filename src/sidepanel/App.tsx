...
async function getActiveConversationFromTabId(tabId: number): Promise<ActiveConversation | null> {
  try {
...
async function waitForConversationDetection(tabId: number): Promise<ActiveConversation | null> {
  let conversation = await getActiveConversationFromTabId(tabId);
  if (conversation) {
    return conversation;
  }
  ...
  for (let attempt = 0; attempt < 6; attempt += 1) {
    ...
    conversation = await getActiveConversationFromTabId(tabId);
    if (conversation) {
      return conversation;
    }
  }
...
async function getActiveConversationFromTab(): Promise<ActiveConversation | null> {
  ...
  return getActiveConversationFromTabId(tabId);
}
...
export function App() {
  ...
  const [extensionEnabled, setExtensionEnabledState] = useState(true);
  ...
  async function refresh(): Promise<ActiveConversation | null> {
  ...
  useEffect(() => {
    ...
    const storageListener: Parameters<typeof chrome.storage.onChanged.addListener>[0] = (
      ...
      if (area === "local" && changes[EXTENSION_SETTINGS_KEY]) {
        const enabledChange = changes[EXTENSION_SETTINGS_KEY];
        const nextValue =
          (enabledChange.newValue as { enabled?: boolean } | undefined)?.enabled ?? true;
        setExtensionEnabledState(nextValue);
        if (!nextValue) {
  ...
  async function handleManualCheck(): Promise<void> {
    ...
    try {
      await waitForConversationDetection(tabId);
      const conversation = await refresh();
      showToast(conversation ? "Chat detected" : "AI chat not detected");
    } finally {
    ...
    <main className="panel-shell">
      <header className="panel-header">
        ...
        <div className="extension-control">
          <label className="current-toggle" htmlFor="extension-enabled-toggle">
            <span>On Page</span>
            <span className={`switch ${extensionEnabled ? "active" : ""}`}>
              <input
                checked={extensionEnabled}
                id="extension-enabled-toggle"
                onChange={(event) => void handleExtensionToggle(event.target.checked)}
                type="checkbox"
              />
        ...
        <div className="panel-title-row">
          ...
          {!activeConversation && extensionEnabled ? (
            <button
              className="retry-detect-button"
              disabled={isManualChecking}
              onClick={() => void handleManualCheck()}
              type="button"
            >
              {isManualChecking ? "Checking..." : "Refresh"}
            </button>
          ) : null}
        </div>
              ...
              <label className="current-toggle" htmlFor="current-chat-toggle">
                ...
                <span className="tooltip-anchor" tabIndex={0}>
                  ...
                  <span className="tooltip-bubble">
                    ?ÑÏû¨ Î≥¥Í≥† ?àÎäî Ï±ÑÌåÖÎ∞©Ïóê???Ä?•Ìïú ?ÄÎß?Î™©Î°ù???úÏãú?©Îãà?? ?ÑÎ©¥ Í∞ôÏ? ?úÎπÑ?§Ïùò ?§Î•∏ Ï±ÑÌåÖÎ∞©Ïóê???Ä?•Ìïú ?Ä???®Íªò Î≥????àÏäµ?àÎã§.
                  </span>
                </span>
